...
void lex(char str[], typeToken t[]){
    ...
    while(str[i] != '\0'){
        if(str[i] == '+'){
            ...
            if(occurence <= 0){
                t[j].lexem = ERROR;
                t[j].value.err.code = 102;
                sprintf(t[j].value.err.message, "Chaine non reconnue\n");
            ...
            char* pt;
            t[j].lexem = REAL;
            t[j].value.real = strtod(tmp, &pt);
        }

        else{
            t[j].lexem = ERROR;
            t[j].value.err.code = 101;
            sprintf(t[j].value.err.message, "Caractere non reconnu\n");
    ...
    t[j].lexem = END;
    t[j].value.err.code = 100;
    sprintf(t[j].value.err.message, "Success\n");
}
